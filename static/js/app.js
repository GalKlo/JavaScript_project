...
d3.json(url).then(function(data) {
    ...
    function init() {
        ...
        for (let i = 0; i < newDataArray.length; i++) {
            if (i == 0) {
            // Create a bar chart
            Plotly.newPlot("bar", newDataArray[i], layout1);   
            }
        else if (i == 1) {
            // Create a bubble chart
            Plotly.newPlot("bubble", newDataArray[i], layout2);
            }
        }
    }
    ...
    dropdownMenu.on("change", function() {
        ...
        for (let i = 0; i < newDataArray.length; i++) {
            if (i == 0) {
                Plotly.react("bar", newDataArray[i],layout1);
            }
            else if (i == 1) {
                Plotly.react("bubble", newDataArray[i],layout2);
            }
            else if (i == 2) {
                metaDataUpdate(sampleMetadata);
            }        
